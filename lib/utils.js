...
exports.namespaceFiles = function(arr, obj) {
  var files = [], len = arr.length;

  for (var i = 0; i < len; i++) {
    var filepath = arr[i];
    // hard-code to basename until we get Strings hooked up
    var basename = path.basename(filepath, path.extname(filepath));
    // var pattern = file[detectPattern(obj.name)](filepath);
    files = files.concat({name: basename, src: filepath});
  }
  return files;
};

exports.namespaceObject = function(arr, objName) {
